const ORIGIN = 'https://admin.lyve.fm'
// const ORIGIN = 'http://localhost:4040'
const MUX_KEY = "15df9e19-1dfa-4a02-9e81-c822e4124889:TRPUR4FCq6VhvSJbzG2Rexmuln+02UWNVOXqwXtlyVYlNG/wifjg15nNjHfF3eVx7EE7t+2Nrc+"
window.onload = function (e) {
    ...
    $(document).ready(function () {
        $('#submit').click(function () { 
            var uri = `${ORIGIN}/api/userSession/verify?token=${token.value}` 
            console.log(uri)
            $.ajax({
                url: uri,
                dataType: 'json',
                ...
                success: function (data) { 
                    if (data.user.role == 'promoter') {
                        var uri = `${ORIGIN}/api/event/fetch-event/${eventId.value}`  
                        $.ajax({
                            url: uri,
                            dataType: 'json',
                            headers: {
                                ...
                                $("#getStream").click(function () {
                                    $.ajax({
                                        ...
                                        headers: {
                                            ...
                                            "Content-Type": "application/json",
                                            "Accept": "*/*",
                                            "Authorization": `Basic ${btoa(MUX_KEY)}`
                                        },
                                        method: 'POST',
                                                    ...
                                                    success: function (comingData) {
                                                        ...
                                                        if (comingData.data.status == 'active') {
                                                            ...
                                                            $("#goLive").click(function () {
                                                                var uri = `${ORIGIN}/api/wallet/create-live-stream?token=${token.value}`
                                                                $.ajax({
                                                                    url: uri,
                                                                    dataType: 'json',
                                                                    headers: {
